...
class Controller extends BaseController
{
    public function getRecomendations($userID)
    {
        ...
        $books = \DB::select("select b.is_book_jsn, b.id, b.title, cnt, prcnt from user_book_histories final inner join book_uniques b on b.id = final.book_id inner join (

    select ubh.user_id,

           count(*) cnt,    (count(*)   /   (select count(*) from user_book_histories where user_id = ubh.user_id)::float)  prcnt
...
group by b.id, b.title, cnt, prcnt
order by prcnt desc ,cnt desc");
